...
function adjustAvailabilityByRange(times: Availables, range: IAvailable): Availables {
  const adjustedTimes = []
...
function mergeAvailabilityByUserToAvailables(availables: Availables, availablesByUser: Availables): Availables {
  ...
  availables[availables.length-1].end = Math.min(availables[availables.length-1].end, mergedRange.end)
  ...
  for (let available of availables) {
    ...
    for (let availableByUser of availablesByUser) {
      ...
      if (available.start === availableByUser.start && available.end === availableByUser.end) {
...
export function getAvailabilityFor(names: string[], dates: string[]): AvailableMap {
  ...
  dates.forEach((date: string) => {
    ...
    names.forEach((name: string, index: number) => {
      ...
      // @ts-ignore
      const availablesByDate = eventByTypeMap.get(EventType.AVAILABLE) as Availables
      // console.log(availableByDate)

      if (index === 0) {
        availables = [...availablesByDate]
      } else {
        availables = mergeAvailabilityByUserToAvailables(availables, availablesByDate)
      }
      // console.log(index, availables)
    })
    
    availableMap.set(date, availables.map((available: IAvailable) => convertTimeFromIntToString(available)))
  })
  return availableMap
}