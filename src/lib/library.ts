...
export function getMergedRange(availables: Availables, availableByUser: Availables): IAvailable {
  const mergedRange: IAvailable = {
    start: Math.max(availables[0].start, availableByUser[0].start),
    end: Math.min(availables[availables.length-1].end, availableByUser[availableByUser.length-1].end)
  }
  return mergedRange
}
...
export function getAvailabilityByDate(busyByDate: Busys): Availables {
  ...
  return available.map((available: IBusy) => convertTimeFromStringToInt(available))
}
